...
class Owner_profile(db.Model, UserMixin):
    ...
    hotel_name = db.Column(db.String(100), nullable=False, unique=True)
...
def home():
    ...
    if current_user.is_authenticated and isinstance(current_user, Owner):
        owner_id = serializer.dumps(current_user.owner_id)# Owner-specific logic 
    ...
    # Convert details to JSON if stored as a string
        for bill in bills:
            ...
            today_income +=bill.total_amount
            
        return render_template('index.html', owner=current_user.owner_name,owner_id=owner_id,bills=bills,total_amount=today_income,todays_bills=todays_bills)
...
def register():
    ...
    # Check if owner already exists
                    ...
                    trial_days = 14  # Set trial period duration
    ...
    if request.method == 'POST':
        ...
        if waiter:
            ...
            if bcrypt.check_password_hash(waiter.password, password):
                login_user(waiter)
                return redirect(url_for('waiter_dashboard'))  # Define waiter-specific dashboard route
            else:
...
def add_hotel_profile():
    ...
    if request.method == 'POST':
        ...
        if not hotel_name or not number_of_tables:
            ...
            return redirect(url_for('hotel_profile'))
    ...
    return render_template('add_hotel_profile.html', profile=profile)
...
def remove_menu_item():
    ...
    return jsonify({'message': 'Item removed successfully.', 'data': session_data}), 200
...
def view_bill():
    ...
    return jsonify({'data': json.loads(session.data)}), 200
...
def complete_bill():
    ...
    new_bill = Bill(
        ...
        details=session.data  # Include ordered items from TableSession
    )
